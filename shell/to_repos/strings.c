...
/**reallocation of memory when size less than quarter of capacity*/
    if(_str->size * 4 <= _str->capacity && _str->capacity > START_STRING_SIZE)/*but capacity can not to be less than 32*/
    {
        _str->capacity /= 2;
...
void string_remove(struct string* _str, long index)
{
    long i = index + 1;
    for(;i < _str->size;++i)
        *(_str->data + i - 1) = *(_str->data + i);/* move elements from [index,size) in left*/
